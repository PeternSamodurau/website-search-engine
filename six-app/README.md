...
4.  Проверьте логи работы в файле `logs/application.log`, чтобы увидеть асинхронное выполнение.

5.  **Для включения многопоточной обработки**, можно раскомментировать строку `# KAFKA_NUM_PARTITIONS: 3` в файле `docker-compose.yml`. Это позволит сервису `order-status-service` читать сообщения из топика `order-topic` в 3 потока.

6.  После включения многопоточности время выполнения под нагрузкой (Postman) уменьшится.
