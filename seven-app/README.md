...
#### 1. Роль: `ROLE_USER` (например, user1@example.com / password1)
...
#### 2. Роль: `ROLE_MANAGER` (manager@example.com / manager)
...
| **`GET /users/{id}`**    | **(Позитивный)** Получение своих данных       | `200 OK`                     | Использовать ID пользователя `user1`         |
| **`GET /users/{id}`**    | **(Позитивный)** Получение чужих данных       | `200 OK`                     | Использовать ID пользователя `user2`         |
...
| **`PUT /users/{id}`**    | **(Негативный)** Обновление чужих данных      | `403 Forbidden`              | Использовать ID пользователя `user2`         |
| **`DELETE /users/{id}`** | **(Позитивный)** Удаление своего профиля      | `204 No Content`             | Использовать ID пользователя `user1`         |
| **`DELETE /users/{id}`** | **(Негативный)** Удаление чужого профиля      | `403 Forbidden`              | Использовать ID пользователя `user2`         |
